...
def update_canvas_image():
    ...
    for mask in masks:
        ...
        for cnt in contours:
            cv2.drawContours(refined_mask, [cnt], -1, color=1, thickness=3)  # 외곽선 두께 3
        mask_total = np.logical_or(mask_total, refined_mask)
...
# tkinter UI 생성
root = tk.Tk()
root.title("Ultralytics SAM 이미지 마스크 에디터 예제")

load_button = tk.Button(root, text="이미지 로드", command=load_image)
load_button.pack(pady=10)

canvas = tk.Canvas(root, bg="white", width=800, height=1200)
canvas.pack()
